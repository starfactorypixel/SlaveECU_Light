...
void HAL_CAN_Send(uint16_t id, uint8_t *data, uint8_t length)
{
	...
	{
		Logger.Printf("CAN ER: %d", hcan.ErrorCode).PrintNewLine();
...
int main(void)
{
	...
	while(1)
	{
		...
		if(current_time - qqq > 5)
		{
			...
			//uint32_t fifoFillLevel = HAL_FDCAN_GetRxFifoFillLevel(&hcan, FDCAN_RX_FIFO0);
...
static void MX_FDCAN1_Init(void)
{
  ...
  hcan.Init.TxFifoQueueElmtsNbr = 16;
...
filterConfig.IdType = FDCAN_STANDARD_ID;     // Стандартный идентификатор
filterConfig.FilterIndex = 0;                // Индекс фильтра
filterConfig.FilterType = FDCAN_FILTER_RANGE; // Тип фильтра: маска
filterConfig.FilterConfig = FDCAN_FILTER_TO_RXFIFO0; // Сообщения в FIFO0
filterConfig.FilterID1 = 0x0000;              // Базовый ID (принимаются все)
filterConfig.FilterID2 = 0x07FF;              // Маска (принимаются все ID)
...
static void MX_USART1_UART_Init(void)
{
  ...
  hDebugUart.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
  if (HAL_UART_Init(&hDebugUart) != HAL_OK)
  {
    Error_Handler();
  }
  if (HAL_UARTEx_SetTxFifoThreshold(&hDebugUart, UART_TXFIFO_THRESHOLD_1_8) != HAL_OK)
  {
    Error_Handler();
  }
  if (HAL_UARTEx_SetRxFifoThreshold(&hDebugUart, UART_RXFIFO_THRESHOLD_1_8) != HAL_OK)
  {
    Error_Handler();
  }
  if (HAL_UARTEx_DisableFifoMode(&hDebugUart) != HAL_OK)
  {
    Error_Handler();
  }
